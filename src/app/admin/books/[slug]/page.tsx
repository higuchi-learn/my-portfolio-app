...
export default function Page() {
	...
	const [isSavingStatus, setIsSavingStatus] = useState<BookStatus | null>(null);
	...
	}, [author, content, description, postSlug, rating, tags, thumbnail, title]);
	...
	useEffect(() => {
		const fetchBook = async () => {
			try {
				const response = await fetch(`/api/admin/books/${slug}`, { cache: "no-store" });
				if (!response.ok) {
				...
				console.error(error);
				setSaveMessage("書籍データの読み込みに失敗しました。");
			} finally {
				setIsLoading(false);
			}
		};
	...
	const savePost = useCallback(async (nextStatus: BookStatus, isAutoSave = false) => {
		if (isSavingStatus) {
			return;
		}
		...
		if (!hasRequiredFields) {
			if (!isAutoSave) {
				setSaveMessage("Slug / Title / Author / Description / Content を入力してから保存してください。");
			}
			return;
		}
		...
		<Container className="flex h-screen min-h-screen flex-col overflow-hidden">
			...
			<Section className="shrink-0 p-sm">
				<Row gap="sm" alignItems="center" justifyContent="space-between">
					<Heading tag="h1" fontClass="title1">Books編集</Heading>
