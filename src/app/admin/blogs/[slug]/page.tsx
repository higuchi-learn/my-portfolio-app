...
export default function Page() {
	...
	const [isSavingStatus, setIsSavingStatus] = useState<BlogStatus | null>(null);
	...
	const savePayload = useMemo(
		...
		[content, description, postSlug, tags, thumbnail, title],
	);
	...
	useEffect(() => {
		const fetchBlog = async () => {
			try {
				const response = await fetch(`/api/admin/blogs/${slug}`, { cache: "no-store" });
				if (!response.ok) {
				...
				console.error(error);
				setSaveMessage("記事データの読み込みに失敗しました。");
			} finally {
				setIsLoading(false);
			}
		};
	...
	const savePost = useCallback(async (nextStatus: BlogStatus, isAutoSave = false) => {
		if (isSavingStatus) {
			return;
		}
		...
		<Container className="flex h-screen min-h-screen flex-col overflow-hidden">
			...
			<Section className="shrink-0 p-sm">
				<Row gap="sm" alignItems="center" justifyContent="space-between">
					<Heading tag="h1" fontClass="title1">Blog編集</Heading>
